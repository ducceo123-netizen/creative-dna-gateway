  ...
  return <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
    ...
    <section className="space-y-3">
      ...
      {filtered.map(p=><article key={p.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-4">
        ...
        {p.status==="accepted" && <div className="flex justify-end"><button onClick={()=>merge(p.id)} disabled={loading} className="px-3 py-2 text-xs font-semibold rounded-lg bg-indigo-600 text-white">Merge to DNA</button></div>}
        {p.merged_at && <div className="text-xs text-emerald-700 font-semibold">Merged {new Date(p.merged_at).toLocaleString()}</div>}
